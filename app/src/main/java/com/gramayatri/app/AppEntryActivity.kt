...
private enum class EntryAppRole {
    DRIVER_OR_ADMIN,
    USER
}

private enum class EntryAuthScreen {
    LOGIN,
    CREATE_ACCOUNT,
    FORGOT_PASSWORD
}
...
class AppEntryActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            var appRole by rememberSaveable { mutableStateOf<EntryAppRole?>(null) }
            var isDriverMode by rememberSaveable { mutableStateOf(false) }
            var authScreen by rememberSaveable { mutableStateOf(EntryAuthScreen.LOGIN) }
            ...
            val openCreateAccount: (String, String) -> Unit = { email, role ->
                ...
                authScreen = EntryAuthScreen.CREATE_ACCOUNT
            }
            ...
            val performLogin: (String, String) -> Unit = { email, password ->
                ...
                authScope.launch {
                    ...
                    if (loginResult.success) {
                        accountExists = true
                        appRole = mapToEntryRole(loginResult.role)
                        authScreen = EntryAuthScreen.LOGIN
                        isDriverMode = false
            ...
            val fusedLocationClient = remember {
                LocationServices.getFusedLocationProviderClient(this@AppEntryActivity)
            }
            val locationTracker = remember {
                FusedLocationTracker(
                    context = this@AppEntryActivity,
                    ...
                    when {
                        appRole == null -> {
                            when (authScreen) {
                                EntryAuthScreen.LOGIN -> {
                                    LoginScreen(
                                        ...
                                        onForgotPasswordClick = {
                                            authError = null
                                            authInfoMessage = null
                                            authScreen = EntryAuthScreen.FORGOT_PASSWORD
                                        }
                                    )
                                }

                                EntryAuthScreen.CREATE_ACCOUNT -> {
                                    CreateAccountScreen(
                                        isLoading = isAuthenticating,
                                        errorMessage = authError,
                                ...
                                EntryAuthScreen.FORGOT_PASSWORD -> {
                        ...
                        selectedRoute == null -> {
                            RouteListScreen(
                                ...
                                showAddRouteAction = appRole == EntryAppRole.DRIVER_OR_ADMIN,
                        ...
                        isDriverMode && appRole == EntryAppRole.DRIVER_OR_ADMIN -> {
                            DriverModeScreen(
                                route = selectedRoute!!,
                            ...
                            BusTrackerScreen(
                                ...
                                showDriverModeAction = appRole == EntryAppRole.DRIVER_OR_ADMIN
                            )
                        }
                    }
                }
...
private fun mapToEntryRole(role: String?): EntryAppRole {
    return when (role?.trim()?.lowercase()) {
        ROLE_DRIVER, ROLE_ADMIN -> EntryAppRole.DRIVER_OR_ADMIN
        else -> EntryAppRole.USER
    }
}
